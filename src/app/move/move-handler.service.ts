...
export class MoveHandlerService {
  ...
  private showGameEndedMessage(winners: Player[]): void {
    ...
    let subTitle =
      winners.length === 1
        ? winnerName + "wins!"
        : winnerName + "tied for first place!";
    // TODO: fixme
  ...
  resolveMergeStocks(
    player: Player,
    mergeInitiator: Player,
    mergeResult: HotelChainMergeResult
  ): Promise<HotelChainMergeResult> {
    var promise;
    var numberOfShares = player.getStockShareForHotelChain(mergeResult.source)
      .quantity;
    if (numberOfShares > 0) {
      console.log("handleMergeStocks for player", player.name);
      promise = this.getMoveHandlerForPlayer(player).handleMergeStocks(
        player,
        mergeResult
      );
    } else {
  ...
  rewardMajorityAndMinorityStockholders(hotelChain: HotelChain): void {
    var majorityStockholders = this.playerService.getMajorityStockholders(
      hotelChain
    );
    var bonus;
    ...
    for (let player of majorityStockholders) {
      bonus =
        this.hotelChainService.getMergeMajorityBonus(hotelChain) /
        majorityStockholders.length;
      player.cash += this.nearestHundred(bonus);
    }

    var minorityStockholders = this.playerService.getMinorityStockholders(
      hotelChain
    );
    for (let player of minorityStockholders) {
      bonus =
        this.hotelChainService.getMergeMinorityBonus(hotelChain) /
