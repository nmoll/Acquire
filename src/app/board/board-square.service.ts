...
export class BoardSquareService {
  ...
  public boardSquares$: Observable<BoardSquare[]>;

  constructor(
    private acquireEventService: AcquireEventService,
    private gameService: GameService
  ) {
    this.boardSquares$ = this.gameService.currentGame$.pipe(
      map(game => game.squares)
    );
  ...
  findSquareById(id: number): BoardSquare {
    return this.getBoardSquares().filter(
      boardSquare => boardSquare.id === id
    )[0];
  }

  getSquareByPosition(positionX: number, positionY: number): BoardSquare {
    var result = this.getBoardSquares().filter(
      square => square.positionX === positionX && square.positionY === positionY
    )[0];
    return result || null;
  }
  ...
  getAdjacentSquares(square: BoardSquare): BoardSquare[] {
    ...
    result.push(
      this.getSquareByPosition(square.positionX - 1, square.positionY)
    );
    result.push(
      this.getSquareByPosition(square.positionX + 1, square.positionY)
    );
    result.push(
      this.getSquareByPosition(square.positionX, square.positionY - 1)
    );
    result.push(
      this.getSquareByPosition(square.positionX, square.positionY + 1)
    );
