...
export class Player {
  ...
  getStockShareForHotelChain(hotelChain: HotelChain) {
    var stockShare = this.stockShares.filter(
      stockShare => stockShare.hotelChain === hotelChain
    )[0];
    if (!stockShare) {
  ...
  addStockShare(stockShare: StockShare) {
    var playerStockShare = this.getStockShareForHotelChain(
      stockShare.hotelChain
    );
