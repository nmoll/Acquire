...
describe("PlayerService", () => {
  ...
  describe("onInit()", () => {
    beforeEach(() => {
      spyOn(acquireEventService.gameEnteredEvent, "subscribe").and.callFake(
        callback => {
          callback();
        }
      );
      spyOn(acquireEventService.gameExitedEvent, "subscribe").and.callFake(
        callback => {
          callback();
        }
      );
  ...
  describe("rotateCurrentPlayer()", () => {
    ...
    describe("if player is FIRST_PERSON and there are multiple FIRST_PERSON players", () => {
      ...
      beforeEach(() => {
        ...
        //     }
        // });
        gameService.currentGame = jasmine.createSpyObj("currentGame", [
          "rotateCurrentPlayer"
        ]);
      ...
      it("should get next player in list of currentPlayer", () => {
        expect(playerService.getNextPlayerInList).toHaveBeenCalledWith(
          currentPlayer
        );
      });
  ...
  describe("getNextPlayerInList()", () => {
    ...
    beforeEach(() => {
      ...
      spyOn(gameService.currentGame, "getNextPlayerInList").and.returnValue(
        nextPlayer
      );
    ...
    it("should ask the current game for the next player", () => {
      expect(gameService.currentGame.getNextPlayerInList).toHaveBeenCalledWith(
        player
      );
    });
  ...
  describe("onGameEntered()", () => {
    ...
    beforeEach(() => {
      tile = new Tile(1, "A1");
      spyOn(acquireEventService.tileSelectedEvent, "subscribe").and.callFake(
        callback => {
          callback(tile);
          return {};
        }
    ...
    it("should subscribe to tileSelectedEvent", () => {
      expect(
        acquireEventService.tileSelectedEvent.subscribe
      ).toHaveBeenCalled();
    });
  ...
  describe("onGameExited()", () => {
    beforeEach(() => {
      playerService.tileSelectedEventSubscription = jasmine.createSpyObj(
        "tileSelectedEventDescription",
        ["unsubscribe"]
      );
    ...
    it("should unsubscribe from tile selected event", () => {
      expect(
        playerService.tileSelectedEventSubscription.unsubscribe
      ).toHaveBeenCalled();
    });
  });
});
