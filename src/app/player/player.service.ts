...
export class PlayerService {
  ...
  onInit() {
    this.acquireEventService.gameEnteredEvent.subscribe(() =>
      this.onGameEntered()
    );
    this.acquireEventService.gameExitedEvent.subscribe(() =>
      this.onGameExited()
    );
  }
  ...
  private promptForTurnStart(player: Player): Promise<any> {
    ...
    if (
      player.playerType == PlayerType.FIRST_PERSON &&
      this.hasMultipleFirstPlayers()
    ) {
      // TODO: fixme
      // let alert = this.alertCtrl.create({
      //     title: `READY, ${player.name}?`,
      //     buttons: [
      //         {
      //           text: "I'm Ready",
      //           handler: (data) => {
  ...
  isCurrentPlayerTile(square: BoardSquare): boolean {
    if (this.getCurrentPlayer().playerType != PlayerType.FIRST_PERSON)
      return false;
  ...
  getMajorityStockholders(hotelChain: HotelChain): Player[] {
    ...
    var stocksOwned = players.map(
      player => player.getStockShareForHotelChain(hotelChain).quantity
    );
  ...
  getMinorityStockholders(hotelChain: HotelChain): Player[] {
    ...
    var minorityStockholders = players.filter(
      player =>
        player.getStockShareForHotelChain(hotelChain).quantity ===
        mostStocksOwned
    );
    if (
      minorityStockholders.length &&
      minorityStockholders[0].getStockShareForHotelChain(hotelChain).quantity >
        0
    ) {
  ...
  orderPlayersByStockShare(hotelChain: HotelChain): Player[] {
    return this.gameService.currentGame.players.sort(function(
      a: Player,
      b: Player
    ) {
  ...
  cashInStocks(player: Player): void {
    for (let stockShare of player.stockShares) {
      player.cash +=
        stockShare.quantity * stockShare.hotelChain.getStockPrice();
    }
  }
  ...
  onGameEntered(): void {
    this.tileSelectedEventSubscription = this.acquireEventService.tileSelectedEvent.subscribe(
      tile => {
