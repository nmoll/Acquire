...
describe("Player", () => {
  ...
  describe("addStockShares()", () => {
    ...
    beforeEach(() => {
      hotelChainAmerican = new HotelChain(
        "American",
        "Am",
    ...
    it("should add all stock shares to player stock shares", () => {
      ...
      expect(
        player.getStockShareForHotelChain(hotelChainAmerican).quantity
      ).toBe(stockShareQuantityAmerican);
      expect(player.getStockShareForHotelChain(hotelChainTower).quantity).toBe(
        stockShareQuantityTower
      );
    });
  });
});
