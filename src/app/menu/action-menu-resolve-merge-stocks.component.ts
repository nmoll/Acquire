...
export class PlayerActionResolveMergeStocksComponent implements OnChanges {
  ...
  constructor(private stockShareService: StockShareService) {}
  ...
  canTrade(): boolean {
    let availableStockShares = this.stockShareService.getAvailableStockShares(
      this.mergeResult.destination
    );

    return (
      this.stockShares >= 2 &&
      availableStockShares - this.stockSharesToTrade >= 1
    );
  }
  ...
  onOk(): void {
    ...
    var mergeSourceStock = this.player.getStockShareForHotelChain(
      this.mergeResult.source
    );
    ...
    var mergeDestinationStock = this.player.getStockShareForHotelChain(
      this.mergeResult.destination
    );
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    this.stockShares = this.player.getStockShareForHotelChain(
      this.mergeResult.source
    ).quantity;
  }
}
