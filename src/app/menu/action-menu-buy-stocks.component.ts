...
export class PlayerActionBuyStocksComponent implements OnInit {
  ...
  canPurchaseStock(stockShare: StockShare): boolean {
    return (
      stockShare.hotelChain.tiles.length > 0 &&
  ...
  private canAffordStock(stockShare: StockShare): boolean {
    return (
      this.getTotalPrice() + stockShare.hotelChain.getStockPrice() <=
      this.playerService.getCurrentPlayer().cash
    );
  }

  private isShareAvailable(stockShare: StockShare): boolean {
    return (
      this.stockShareService.getAvailableStockShares(stockShare.hotelChain) -
        stockShare.quantity >
      0
    );
  }
