...
export class HotelChainService {
  ...
  getActiveHotelChains(): HotelChain[] {
    return this.getHotelChains().filter(
      hotelChain => hotelChain.tiles.length > 0
    );
  }
  ...
  findAllByTiles(tiles: Tile[]) {
    var hotelChains = tiles.map(t => t.hotelChain).filter(t => !!t);
  ...
  init(): HotelChain[] {
    var hotelChains = [];
    hotelChains.push(
      new HotelChain("Worldwide", "WOR", HotelChainType.WORLDWIDE)
    );
    hotelChains.push(new HotelChain("Luxor", "LUX", HotelChainType.LUXOR));
    hotelChains.push(
      new HotelChain("Festival", "FES", HotelChainType.FESTIVAL)
    );
    hotelChains.push(
      new HotelChain("Imperial", "IMP", HotelChainType.IMPERIAL)
    );
    hotelChains.push(
      new HotelChain("American", "AME", HotelChainType.AMERICAN)
    );
    hotelChains.push(
      new HotelChain("Continental", "CON", HotelChainType.CONTINENTAL)
    );
